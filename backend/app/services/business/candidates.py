...
class BusinessCandidatesMixin:
    ...
    def create_candidate(
        ...
        pa_setup_id = cls._validated_pa_setup_id(session, request.pa_setup_id)
        ...
        cls._audit(session, principal, "candidate.create", "candidate", candidate.candidate_id)
        session.commit()
        session.refresh(candidate)
        return cls._candidate_response(session, candidate)
    ...
    def list_candidates(
        ...
        statement = cls._candidate_list_statement(
            principal=principal,
            decision=decision,
        ...
        return [cls._candidate_response(session, row) for row in rows]
    ...
    def get_candidate_detail(
        ...
        pa_setup = cls._candidate_pa_setup(session, candidate)
        entry_plan = pa_setup.entry_plan if pa_setup else None
        strat_signal = cls._candidate_strat_signal(session, candidate, pa_setup)
        strat_plan = cls._candidate_strat_plan(session, candidate, pa_setup)
        return CandidateDetail(
            candidate=cls._candidate_response(session, candidate, pa_setup),
            ...
            scanner_decision=cls._candidate_scanner_decision(candidate, entry_plan),
    ...
    def preview_candidate_plan(
        ...
        portfolio_before = cls._portfolio_risk_summary(session, principal)
        ...
        preview_item = cls._preview_portfolio_risk_item(
            candidate=candidate,
        ...
        portfolio_after_plan = cls._portfolio_risk_summary(
            session,
            principal,
    ...
    def update_candidate(
        ...
        if "pa_setup_id" in payload:
            payload["pa_setup_id"] = cls._validated_pa_setup_id(
                session,
                payload["pa_setup_id"],
            )
        ...
        if payload:
            cls._audit(session, principal, "candidate.update", "candidate", candidate_id)
            session.commit()
            session.refresh(candidate)
        return cls._candidate_response(session, candidate)
    ...
    def _candidate_response(
        ...
        setup = pa_setup if pa_setup is not None else cls._candidate_pa_setup(session, candidate)
    ...
    @classmethod
    def _candidate_pa_setup(cls, session: Session, candidate: db.Candidate) -> db.PASetup | None:
        setup_id = candidate.pa_setup_id or cls._legacy_pa_setup_id(candidate)
        if not setup_id:
            return None
    ...
    def _candidate_scanner_decision(
        ...
        if isinstance(scanner_decision, dict):
            return cls._normalize_scanner_decision(scanner_decision)
        if not candidate.ai_review_json:
            return None
        try:
            payload = json.loads(candidate.ai_review_json)
        ...
        return (
            cls._normalize_scanner_decision(scanner_decision)
            if isinstance(scanner_decision, dict)
            else None
        )
