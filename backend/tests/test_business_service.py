...
def test_candidate_detail_synthesizes_scanner_decision_for_legacy_setup(session) -> None:
    principal = _principal("user_a", "acct_a")
    session.add(
        db.PASetup(
            setup_id="pasetup_legacy",
            symbol_id="SOXX",
            timeframe="1d",
            detected_ts=datetime(2026, 4, 26, tzinfo=UTC),
            setup_type="oneil_leader_watch",
            setup_grade="A",
            pa_quality_score=85,
            entry_plan={
                "trigger_price": 450.14,
                "score_breakdown": {
                    "trend": 25,
            ...
            exit_plan={"initial_stop": 410.5},
            invalidation={"price_below": 410.5},
            validation_status="shadow_only",
            status="candidate",
        )
    )
    BusinessService.create_candidate(
        ...
        CandidateCreate(
            ...
            strategy_name="oneil_core_us_etf",
            setup_type="oneil_leader_watch",
            pa_setup_id="pasetup_legacy",
        ),
    )

    detail = BusinessService.get_candidate_detail(session, principal, "cand_legacy")

    assert detail.scanner_decision is not None
    assert detail.scanner_decision["version"] == "legacy_score_breakdown_fallback"
    assert detail.scanner_decision["decision"] == "candidate"
    assert detail.scanner_decision["passed_rules"]
